...
<?php   
 ...
 $query = "SELECT  CONCAT( pd.fname,' ',pd.lname) AS patient, pd.email AS email,p.mphone AS mphone , p.id AS pid, pd.image AS image,  DATE_FORMAT(a.time_from,'%d-%m-%Y') AS appointment_date,  DATE_FORMAT(a.time_from,'%r') AS time_from ,  DATE_FORMAT(a.time_to,'%r') AS time_to , a.description AS description, DATE_FORMAT( a.date ,'%d-%m-%Y %r')AS date_time , a.status AS status, a.delete_status AS delete_status  FROM `appointment`a LEFT JOIN patient p ON a.patient_id = p.id LEFT JOIN patient_details pd ON p.patient_details_id = pd.id WHERE        NOW() <=  a.time_from  AND a.delete_status=0  AND a.doctor_id = ".$idxd." AND a.status =0 ORDER BY a.time_from ";
                    ...
                    <div class="col-md-3 back_white pading_5 date_details">
                        ...
                        <form class="form-horizontal  ">
                                ...
                                <div class="form-group  ">
                                   ...
                                   <p class="col-sm-12 control-label text-left" style="color:'.$stat_color.';">'.$statu .'</p>
                                </div>
...
try {
 ...
 $query = "SELECT  CONCAT( pd.fname,' ',pd.lname) AS patient, pd.email AS email,p.mphone AS mphone , p.id AS pid, pd.image AS image,  DATE_FORMAT(a.time_from,'%d-%m-%Y') AS appointment_date,  DATE_FORMAT(a.time_from,'%r') AS time_from ,  DATE_FORMAT(a.time_to,'%r') AS time_to , a.description AS description, DATE_FORMAT( a.date ,'%d-%m-%Y %r')AS date_time , a.status AS status, a.delete_status AS delete_status  FROM `appointment`a LEFT JOIN patient p ON a.patient_id = p.id LEFT JOIN patient_details pd ON p.patient_details_id = pd.id WHERE a.doctor_id = ".$idxd." ORDER BY a.time_from ";
             $result = $a->display($query);
